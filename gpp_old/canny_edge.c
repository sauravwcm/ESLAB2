                   ...
                   short int *magnitude)
{
    ...
    {
        dx[0]=(float)delta_x[i];
        dx[1]=(float)delta_x[i+1];
        dx[2]=(float)delta_x[i+2];
        dx[3]=(float)delta_x[i+3];
        dy[0]=(float)delta_y[i];
        dy[1]=(float)delta_y[i+1];
        dy[2]=(float)delta_y[i+2];
        dy[3]=(float)delta_y[i+3]; 
        vec1= vld1q_f32 (dx); 
        vec2= vld1q_f32 (dy);
        vec1= vmulq_f32(vec1,vec1);
        vec2= vmulq_f32(vec2,vec2);
        vec1= vaddq_f32(vec1,vec2);
        vec2= vmovq_n_f32(0.5);     
        vec1=sqrtv(vec1);
        vec1= vaddq_f32(vec1,vec2);
        vst1q_f32 (dx, vec1);
        magnitude[i]= (short)dx[0];
        magnitude[i+1]= (short)dx[1];
        magnitude[i+2]= (short)dx[2];
        magnitude[i+3]= (short)dx[3];
    }
}
...
short int* gaussian_smooth(unsigned char *image, int rows, int cols, float sigma)
{
  ...
  short int* smoothedim;
    ...
    ****************************************************************************/
    ...
    for(r=0+PART-center; r<rows; r++)   //PART number of rows smoothed on DSP
    {
        for (c = 0; c < center; c++)
        {
            dot = 0.0;
        ...
        for(c=center; c<cols-center; c++)
        {
            ...
            {
                im[0]= (float)image[r*cols+(c+cc)];   
                im[1]= (float)image[r*cols+(c+cc+1)];
                im[2]= (float)image[r*cols+(c+cc+2)];
                im[3]= (float)image[r*cols+(c+cc+3)];

                kr[0]=  kernel[cc+center];
                kr[1]=  kernel[cc+center+1];
                kr[2]=  kernel[cc+center+2];

                if (cc<=(center-4))
                {
                    kr[3]=  kernel[cc+center+3];
                }
                else
                {
                    kr[3]=0;
    ...
    ****************************************************************************/
    ...
    free(tempim);
    free(kernel);
    printf("Gaussian smooth done on ARM \t"); timeCheck();
    sync();															// Wait for DSP
    printf("buf[24] : %d \n",pool_notify_DataBuf[24]);
    printf("buf[25] : %d \n",pool_notify_DataBuf[25]);
    printf("buf[26] : %d \n",pool_notify_DataBuf[26]);
    printf("buf[27] : %d \n",pool_notify_DataBuf[27]);

    printf("buf[24] after conversion : %f \n",FX2FLOAT(pool_notify_DataBuf[24]));

    comb_smoothedim(smoothedim, cols);								// Combine smoothedim created by DSP and ARM
    printf("smoothedim[24] : %d\n", smoothedim[24]);
    return smoothedim;
}
